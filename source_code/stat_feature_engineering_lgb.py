...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb  1 03:18:51 2020
...
from traj_data_classification import lgb_clf_embedding_list_train
...
def traj_coord_stat(traj=None):
    ...
    stat_feature_fcn = [np.mean, np.ptp]
...
#    # Step 2: Basic semantic coordinate features
#    stat_feature_fcn = [np.std, np.median, np.ptp]
#    for col_name in ["lon", "lat"]:
#        for fcn in stat_feature_fcn:
...
#
#    # Step 3: Boat activity area
#    quantile_low, quantile_high = [0.01], [0.99]
#    feature_names.extend(["base_activity_area_{}_{}".format(
#            i, j) for i, j in zip(quantile_low, quantile_high)])
#    feature_vals.extend(traj_activity_area_quantile(traj,
...
#                                                    quantile_high=quantile_high))
    ...
    # Step 5: Quantile features(0.02, 0.99, 27)
    quantile = np.linspace(0.02, 0.99, 27)
...
#    quantile = np.linspace(0.02, 0.99, 25)
#    feature_names.extend(["base_lon_quantile_{}".format(i) for i in quantile])
#    feature_vals.extend(traj_quantile_features(traj, quantile=quantile,
#                                               feature_name="lon"))
#
#    quantile = np.linspace(0.02, 0.99, 25)
#    feature_names.extend(["base_lat_quantile_{}".format(i) for i in quantile])
#    feature_vals.extend(traj_quantile_features(traj_move, quantile=quantile,
#                                               feature_name="lat"))
...
def traj_speed_stat(traj=None):
    ...
    traj_move = traj[traj["is_stop"] == -1]
#    traj_stop = traj[traj["is_stop"] != -1]
#    traj["speed"][traj["is_stop"] != -1] = 0
    feature_vals, feature_names = [], []

#    # Step 1: Basic statistics
#    stat_feature_fcn = [np.mean, np.std]
...
#                feature_vals.append(0)
    ...
    # Step 1: Quantile features(0.02, 0.98, 25)
    quantile = np.linspace(0.02, 0.98, 43)
    feature_names.extend(["base_speed_quantile_{}".format(i) for i in quantile])
    feature_vals.extend(traj_quantile_features(traj_move, quantile=quantile,
                                               feature_name="speed"))

    # Step 2: Histogram of speed(7, (0, 5))
...
def traj_dir_stat(traj=None):
    ...
    traj_move = traj[traj["is_stop"] == -1]
    traj_stop = traj[traj["is_stop"] != -1]
    traj["direction"][traj["is_stop"] != -1] = 366
    feature_vals, feature_names = [], []
...
#                                               feature_name="direction"))
    ...
    hist_vals, hist_cut = np.histogram(traj["direction"].values, bins=num_bins,
                                       range=bin_range, normed=True)
...
def traj_expert(traj=None):
    ...
    # Step 1: Coordinate divergence features[0.1, 0.13, 0.15]
...
############################# Training part #############################
def stat_feature_engineering_lgb():
    ...
    # Step 5: Signal embedding vector
    dir_embedding = traj_data_direction_embedding(total_data, embedding_size=5,
                                                  iters=40, window_size=25,
                                                  min_count=3)
    total_features = pd.merge(total_features, dir_embedding, on="boat_id",
                              how="left")

    speed_embedding = traj_data_speed_embedding(total_data, embedding_size=5,
                                                iters=40, window_size=25,
                                                min_count=3)
    ...
    file_processor = LoadSave()
    file_processor.save_data(path=".//tcdata_tmp//train_feature_lgb.pkl",
                             data=train_feature)
    file_processor.save_data(path=".//tcdata_tmp//train_target.pkl",
                             data=train_feature[["boat_id", "target"]])
    file_processor.save_data(path=".//tcdata_tmp//test_feature_lgb.pkl",
                             data=test_feature)
    gc.collect()
...
#    gc.collect()
    ...
    for embedding_id in [0, 1, 2]:
        lgb_res_list = lgb_clf_embedding_list_train(folds=5, id_list=[embedding_id],
                                                    embedding_enable=True)
        df = training_res_to_log(training_res=lgb_res_list[0][0], comment="lgb_{}".format(embedding_id))
